...
5. НУМЕРАЦИЯ — ТОЛЬКО ВНУТРИ ЗАТРОНУТОГО РАЗДЕЛА. Добавил или удалил пункт — пересчитай номера последующих пунктов ЭТОГО ЖЕ раздела и верни их через REPLACE. Номера самих разделов и пункты ДРУГИХ разделов не трогай никогда, даже если их нумерация кажется тебе неправильной или непоследовательной: в договоре бывает своя логика, а лишняя перенумерация ломает ссылки между пунктами. Сохраняй тот стиль оформления, который уже принят в документе («4.1.», «4.1 », «4)» и т. п.) — не приводи его к своему.
...
8. Если пользователь просит ИЗМЕНИТЬ существующие пункты — правь именно их через REPLACE. ЗАПРЕЩЕНО вместо этого дописывать новый раздел с похожим содержанием: в договоре появятся два противоречащих друг другу места.
9. Если ты не нашёл в документе того, о чём просит пользователь (нет такого пункта, раздела или формулировки) — НЕ ПРИДУМЫВАЙ и не создавай новых разделов. Верни строку:
<<<NOT_FOUND>>> короткое пояснение, что именно не найдено и что уточнить у пользователя
Это нормальный ответ: лучше переспросить, чем испортить договор.
...
/**
 * Сколько символов договора помещается в один запрос к модели.
 *
 * Правки идут через модель со 128k контекста: 120 000 знаков русского текста —
 * это примерно 45-50 тысяч токенов, плюс инструкции и место под ответ. Запас
 * сознательный: упереться в предел контекста — значит получить обрезанный или
 * бессвязный ответ на договоре клиента.
 *
 * Документ длиннее лимита в один запрос не влезает; что происходит в этом
 * случае — см. selectBlocksForPrompt и предупреждение пользователю в
 * api/versions/[id]/chat.
 */
export const EDIT_PROMPT_CHAR_LIMIT = Number(process.env['AI_EDIT_MAX_PROMPT_CHARS'] ?? 120_000)
