...
// Базовая «личность» для диалоговых задач. Отдельно от правил формата ответа —
// формат зависит от задачи, личность нет.
...
/**
 * Правила ответа в режимах «Вопрос» и «Анализ».
 *
 * Зачем так жёстко: на живом документе модель в ответ на «найди ошибки в
 * приложении» переписала в чат всю таблицу со всеми позициями и суммами.
 * Это не только нечитаемо — это обход платных правок: попросив «перепиши пункт
 * целиком», пользователь получал готовый текст бесплатно и вставлял его в
 * договор мимо режима «Правка». Поэтому запрет на выдачу готовых формулировок
 * и на воспроизведение документа — часть монетизации, а не стилистика.
 *
 * Требование коротких абзацев и списков — следствие того, что пузырь чата узкий
 * и «полотно» в нём нечитаемо.
 */
const CHAT_ANSWER_RULES = [
  '══════════ КАК ОТВЕЧАТЬ ══════════',
  'Ты отвечаешь на вопрос о документе. Ты НЕ редактируешь документ в этом режиме.',
  'Схема ответа по каждой проблеме — четыре коротких элемента, в этом порядке:',
  '  1) ГДЕ — номер пункта, раздела или строки таблицы;',
  '  2) ЧТО НЕ ТАК — суть проблемы одним предложением;',
  '  3) ПОЧЕМУ — чем это грозит пользователю на практике;',
  '  4) ЧТО ДЕЛАТЬ — конкретное действие (без готового текста формулировки).',
  'Пиши короткими абзацами и списком: одна проблема — один пункт списка из 1–3 предложений.',
  'Ответ целиком — до 300 слов. Если проблем много, назови 5–7 самых важных и допиши, сколько осталось.',
  'Не пересказывай содержание документа и не подводи итогов «о чём договор», если об этом не спросили.',
  '',
  '══════════ ЗАПРЕЩЕНО ══════════',
  '  ✗ Воспроизводить текст договора, пункта, раздела или таблицы целиком — даже если пользователь просит.',
  '  ✗ Перепечатывать таблицы построчно. Из таблицы называй ТОЛЬКО проблемные строки.',
  '  ✗ Выдавать готовую формулировку для вставки в договор («вот новая редакция пункта…»).',
  '  ✗ Спрашивать у пользователя, где ошибки, или предлагать ему указать их самому — ищи сам.',
  '  ✗ Отвечать «всё в порядке», не назвав, что именно проверил.',
  'Цитировать разрешено только короткий фрагмент — до 15 слов — и только чтобы показать, о каком месте речь.',
  '',
  'Если просят переписать пункт, составить или дать формулировку, добавить или удалить условие —',
  'НЕ пиши текст в чат. Коротко объясни, что и почему нужно изменить, и добавь: внести правку',
  'в документ можно в режиме «Правка» в этом же чате — это платное действие, тратится правка из пакета.',
].join('\n')

/**
 * Алгоритм проверки таблиц — на случай, когда таблицу считает модель.
 * Основную арифметику считает код (lib/table-math.ts) и подаёт результат фактами;
 * этот блок нужен для таблиц, которые парсер не распознал (нестандартная шапка,
 * объединённые ячейки).
 */
const TABLE_CHECK_RULES = [
  '══════════ ПРОВЕРКА РАСЧЁТОВ В ТАБЛИЦАХ ══════════',
  'Если в документе есть таблица с числами, проверяй её ПОСТРОЧНО, а не по итогу:',
  '  1) для каждой строки пересчитай: Сумма = Кол-во × Цена × Коэффициент (коэффициента может не быть — тогда ×1);',
  '  2) сравни полученное с суммой, указанной в строке;',
  '  3) сложи суммы всех строк раздела и сравни со строкой «Итого» этого раздела;',
  '  4) сложи итоги разделов и сравни с общим итогом и с суммой договора в тексте.',
  'Расхождения выводи списком: «строка → ожидалось → указано → разница».',
  'Совпадение итога НЕ доказывает, что строки верны: ошибка в строке может быть скомпенсирована итогом.',
  'Не переспрашивай, где ошибки, и не предлагай пользователю проверить самому — считай сам.',
].join('\n')
...
function toGigachatMessages(messages: AIMessage[], settings: AISettings, documentText: string) {
  ...
  const systemParts = [buildSystemPrompt(settings), CHAT_ANSWER_RULES, TABLE_CHECK_RULES]

  // Арифметика считается детерминированно и подаётся как готовый факт: модель
  // на этих числах не ошибётся и не начнёт сомневаться в собственном пересчёте.
  // Проверка вспомогательная — её падение не должно ломать ответ на вопрос.
  if (documentText) {
    try {
      const report = formatTableMathReport(checkDocumentTables(documentText))
      if (report) systemParts.push(report)
    } catch {
      // Таблица нестандартная — считает модель по TABLE_CHECK_RULES
    }
...
export const gigachatProvider: AIProvider = {
  async *chat(messages: AIMessage[], settings: AISettings, documentText: string) {
    ...
    const payload = {
      ...
      // 3000 токенов ≈ 2000 слов — с запасом на список из 5–7 проблем с
      // пояснениями (по правилам ответа лимит 300 слов) и на длинный перечень
      // арифметических расхождений. При этом переписать в чат целиком таблицу
      // приложения или раздел договора в такой лимит уже не получится —
      // прежние 32768 разрешали ровно это.
      max_tokens: 3000,
      repetition_penalty: 1,
  ...
  async review(documentText: string, settings: AISettings): Promise<ReviewResult> {
    ...
    const systemContent = [
      ...
      '',
      `БЛОК 8 — АРИФМЕТИКА ТАБЛИЦ И СУММ (category: "finance")`,
      'Проверяй расчёты ПОСТРОЧНО, а не по итоговой сумме — ошибка в строке может быть',
      'скомпенсирована подогнанным итогом, и «итог сходится» ничего не доказывает:',
      '  1) по каждой строке пересчитай Сумма = Кол-во × Цена × Коэффициент (коэффициента может не быть — тогда ×1);',
      '  2) сравни результат с суммой, указанной в строке;',
      '  3) сложи строки раздела и сверь со строкой «Итого» раздела;',
      '  4) сложи итоги разделов и сверь с общим итогом и с ценой договора в тексте;',
      '  5) сверь сумму прописью с суммой цифрами, а ставку НДС — с расчётом.',
      'Каждое расхождение — отдельный issue: в description укажи строку, ожидаемое и указанное значение и разницу.',
      '  Расхождение в деньгах → severity: "risk", importance: "high", clause: номер приложения и строки',
      '  ЗАПРЕЩЕНО спрашивать у пользователя, где ошибки, — находи сам.',
      '  ЗАПРЕЩЕНО писать, что расчёты верны, не пересчитав строки.',
      '',
    ...
    // Арифметику таблиц считаем кодом по ПОЛНОМУ тексту документа и подаём модели
    // готовым фактом. Это важно вдвойне: ниже длинный документ обрезается и
    // приложения могут не попасть в текст для анализа — числа из них дойдут сюда
    // в любом случае. Проверка вспомогательная: её сбой не должен ронять анализ.
    let tableMathBlock = ''
    try {
      tableMathBlock = formatTableMathReport(checkDocumentTables(documentText))
    } catch {
      tableMathBlock = ''
    }
    const systemWithMath = tableMathBlock ? `${systemContent}\n\n${tableMathBlock}` : systemContent
    ...
    const prompt = [
      ...
      'issues: от 10 до 18 пунктов — охватывай ВСЕ 8 блоков анализа, не только общие условия',
    ...
    if (documentText.length > MAX_DOC_CHARS) {
      ...
      if (appendixStart > 10_000 && appendixStart < MAX_DOC_CHARS) {
        // Раньше приложения выбрасывались целиком — и ошибки в них (в первую
        // очередь в суммах) невозможно было найти в принципе. Теперь основной
        // договор берётся полностью, а остаток бюджета отдаётся началу
        // приложений: лимит в 40К соблюдён, но приложения хотя бы попадают в анализ.
        // Числа из приложений в любом случае уже посчитаны кодом (см. tableMathBlock).
        const main = documentText.slice(0, appendixStart)
        const appendix = documentText.slice(appendixStart, MAX_DOC_CHARS)
        docForReview = main + appendix +
          '\n\n[Приложения включены частично — далее текст обрезан по объёму. Расчёты по таблицам уже проверены программно, см. блок проверенных расчётов]'
      } else {
    ...
    const content = await completeText({
      ...
      messages: [
        { role: 'system', content: systemWithMath },
  ...
  async *generate(
    ...
    const structureRequirement = isChildDoc
      ? [
          'СТРУКТУРА ПРИЛОЖЕНИЯ / ДОПСОГЛАШЕНИЯ — КОМПАКТНО (это НЕ полный договор!):',
          'НЕ пиши шапку/преамбулу (строку «Приложение № … к Договору …», город, дату, представление сторон «в лице …, именуемое «Заказчик»… заключили…») — её вставляет СИСТЕМА. Начинай СРАЗУ с «1. ПРЕДМЕТ».',
          'НЕ пиши в конце раздел «Реквизиты и подписи сторон», ИНН, счета, подписи — их добавляет СИСТЕМА.',
          'Разделы (только по сути, коротко):',
          '1. Предмет (что регулирует приложение/ДС; явная ссылка на пункт/раздел основного договора, например «в соответствии с п. 1.1 Договора»)',
          '2. Согласованные условия / смета (если есть цены — оформи ТАБЛИЦЕЙ <table> с итоговой строкой)',
          '3. Стоимость и порядок оплаты (если применимо)',
          '4. Порядок вступления в силу и прочие условия (кратко)',
          'ПИШИ КОМПАКТНО: короткие пункты по делу, без длинных вводных и повторов. Не раздувай объём — приложение/ДС обычно в разы короче основного договора.',
        ].join('\n')
      : [
  ...
  async detectHeadings(candidates: string[]): Promise<{ title: number | null; headings: number[] }> {
    ...
    const prompt = [
      ...
      '- НЕ заголовки: подпункты («1.1.», «2.3.4.», «а)», «б)»), обычный текст условий, строки с реквизитами (ИНН/ОГРН/БИК/счёт), город и дата, представление сторон («… в лице …, действующего …»).',
      '- НЕ заголовки: строки подписей и ФИО с прочерком («Иванов И.И. ______», «Генеральный директор»), поля форм/заявок и строки с прочерками для заполнения («Дата: ____», «отметить нужное»), служебные пометки («ОБРАЗЕЦ», «Приложение №1», «к Договору …»).',
      '- Заголовок — это короткая строка-НАЗВАНИЕ части договора, а не предложение с условием, не подпись и не поле для заполнения.',
      '- Разделов в договоре обычно немного (примерно 5–15). Не помечай десятки строк — выбирай только настоящие заголовки разделов.',
