...
import { sanitizeHtml, normalizeLegalHtml } from '@/lib/html-document'
...
export function DecorEditor({ html, hint, editable, onActivate, onSave }: DecorEditorProps) {
  ...
  const [initialContent] = useState(() => normalizeLegalHtml(sanitizeHtml(html)))
  ...
  useEffect(() => {
    ...
    const next = normalizeLegalHtml(sanitizeHtml(html))
