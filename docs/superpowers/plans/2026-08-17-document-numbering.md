...
> **Статус на 07.09.2026: план выполнен полностью.** Галочки проставлены задним числом —
> при исполнении их не отмечали, и план выглядел нетронутым. Сверено по коду: ядро,
> обе ручки API, колонка с миграцией, общий компонент и все четыре точки подстановки
> на месте. Дополнительно закрыт пробел, которого в плане не было: на ядро написаны
> тесты (`apps/web/tests/document-number.test.ts`) — план проверял его одноразовым
> скриптом и удалял скрипт, из-за чего модуль оставался без защиты в CI.


> **Для агентов-исполнителей:** используйте superpowers:subagent-driven-development или superpowers:executing-plans, задача за задачей. Шаги помечены чекбоксами `- [x]`.
...
- [x] **Шаг 1: Создать модуль целиком**
...
- [x] **Шаг 3: Прогнать логику вручную одним скриптом**
...
- [x] **Шаг 4: Удалить временный скрипт и закоммитить**
...
- [x] **Шаг 1: Добавить колонку и индекс в схему**
...
- [x] **Шаг 3: Принять поле в схемах валидации**
...
- [x] **Шаг 4: Проверить типы и закоммитить**
...
- [x] **Шаг 1: Роут следующего номера**
...
- [x] **Шаг 3: Поиск по номеру и фильтр по родителю**
...
- [x] **Шаг 1: Добавить поле в состояние формы**
...
- [x] **Шаг 2: Починить автоподстановку номера приложения**
...
- [x] **Шаг 1: Начать передавать profileId**
...
- [x] **Шаг 2: Заменить голые инпуты номера**
...
- [x] **Шаг 1: Пройти сценарий из спецификации**
...
- [x] **Шаг 3: Добавить фазу 16 в CLAUDE.md**
